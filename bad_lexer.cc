    ...
    if (isdigit(c) || ( c == 'A' || c == 'B' || c == 'C' || c == 'D' || c == 'E' || c == 'F')) {
        ...
        if (!input.EndOfInput()){
            ...
            if (c == '.'){
                ...
                while (!input.EndOfInput()) { // Goes through adding to lexeme after '.'
                    tmp.lexeme += c;
                    input.GetChar(c);
                }
        ...
        //  ---------------------------------------------------- REALNUM
            ...
            if (c == 'x'){  
                ...
                if((c != '1')){    // if 0 is does not follow an x, it will immediately quit
                    ...
                    current += c;
                    check = prior + current;
                    input.UngetString(check);
                    input.GetChar(c);
                    // cout << c << endl;
                    // cout << tmp.lexeme << endl;
                ...
                input.GetChar(c);
                    ...
                    if((c != '6')){     // if 8 does not follow the 0, it will immediately quit
                                        // Because it is not a base 16, any alpha characters need to be considered
                                        // alpha characters and not digits
